...
#endif
{
	...
	pwlan_dev = alloc_netdev_mq(sizeof(hdd_adapter_t),
         ...
         pwlan_dev = alloc_netdev_mq(sizeof(hdd_adapter_t), name, NET_NAME_UNKNOWN, ether_setup, NUM_TX_QUEUES);
#endif
...
#endif
{
	...
	{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4,12,0))
               adapter = hdd_alloc_station_adapter(hdd_ctx, mac_addr, iface_name);
#else
