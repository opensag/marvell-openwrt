...
hif_pci_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
    ...
    u_int32_t lcr_val;
    //u_int16_t pci_config;
    u_int32_t reg = 0, flag = 1;
    printk(KERN_INFO "%s:, con_mode= 0x%x\n", __func__, vos_get_conparam());
    
    addr = ioremap(0xd0013810, 1);
    reg = readl(addr);
    if ((reg & 0xe00000) == 0x800000 || (reg & 0xe00000) == 0x600000) {
        flag = 1;
    }
    iounmap(addr);

    if (flag == 0) {
        addr = ioremap(0xd0076208, 1);
        reg = readl(addr);
        writel(reg&(~(1<<30)), addr);
        iounmap(addr);
    } else {
        addr = ioremap(0xd0076208, 1);
        reg = readl(addr);
...
#define BAR_NUM 0
    ...
    //pci_read_config_word(pdev, 0x78, &pci_config);
    //pci_write_config_word(pdev, 0x78, pci_config & ~ 0x0010);
    //pci_read_config_word(pdev, 0x78, &pci_config);
    //printk("---------pci_write_config_word 0x78=%x--------------\n", pci_config);
