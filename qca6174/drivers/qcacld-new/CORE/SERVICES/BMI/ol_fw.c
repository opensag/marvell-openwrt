...
static struct non_qc_platform_pci_fw_files FW_FILES_QCA6174_FW_3_1 = {
PREFIX "athwlan30.bin", PREFIX "bdwlan30.bin", PREFIX "otp30.bin", PREFIX "utf30.bin",
PREFIX "utfbd30.bin", PREFIX "epping30.bin", PREFIX "evicted30.bin"};
...
int get_fw_files_for_non_qc_pci_target(struct non_qc_platform_pci_fw_files *pfw_files,
                           u32 target_type, u32 target_flag, u32 target_version)
{
	if (!pfw_files)
		return -ENODEV;
	...
	switch (target_version) {
		...
		case QCA9379_REV1_VERSION:
                        if (target_flag) {
			    memcpy(pfw_files, &FW_FILES_QCA6174_FW_3_1,
				           		sizeof(*pfw_files));
                        } else {
...
int ol_download_firmware(struct ol_softc *scn)
{
	...
	A_STATUS ret;
		...
		if (0 != get_fw_files_for_non_qc_pci_target(&scn->fw_files,
						scn->target_type, scn->target_flag,
						scn->target_version)) {
