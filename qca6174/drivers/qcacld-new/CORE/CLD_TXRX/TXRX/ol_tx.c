    ...
    u_int16_t chanfreq)
{
    ...
    if (! tx_desc) {
        TXRX_PRINT(TXRX_PRINT_LEVEL_ERR, "can't accept the tx mgmt frame, tx_desc is null %d", pdev->cfg.is_high_latency);
    ...
    if (pdev->cfg.is_high_latency) {
        ...
        if (!txq) {
            ...
            TXRX_PRINT(TXRX_PRINT_LEVEL_ERR, "can't accept the tx mgmt frame, txq is null %d", pdev->cfg.is_high_latency);
            return 1; /* can't accept the tx mgmt frame */
        }
         /* Initialize the HTT tx desc l2 header offset field.
